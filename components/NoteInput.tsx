...
const TYPES: { value: SourceType; emoji: string; label: string; color: string }[] = [
  { value: 'note', emoji: '📝', label: 'Note', color: 'from-violet-500 to-purple-600' },
  { value: 'idea', emoji: '💡', label: 'Idea', color: 'from-amber-400 to-orange-500'  },
  { value: 'doc',  emoji: '📄', label: 'Doc',  color: 'from-sky-500 to-blue-600'      },
]

export default function NoteInput({ userId, onSaved }: { userId: string; onSaved: () => void }) {
  ...
  return (
    ...
    >
      {/* Gradient top bar */}
      <div className={`h-1.5 rounded-t-3xl bg-gradient-to-r ${active.color} transition-all duration-300`} />
      ...
      <div className="p-5">
        ...
        <div className="flex items-center justify-between mb-3">
          <span className="text-xs text-gray-300">
            {content.length > 0 ? `${content.length} chars` : 'or speak your memory'}
          </span>
          <VoiceInput
        ...
        {tags && (
          <div className="flex flex-wrap gap-1.5 mb-3">
            {tags.split(',').map(t => t.trim()).filter(Boolean).map(tag => (
              <span key={tag} className="text-xs bg-violet-50 text-violet-600 px-2.5 py-1 rounded-full border border-violet-100 font-medium">
                {tag}
              </span>
            ))}
          </div>
